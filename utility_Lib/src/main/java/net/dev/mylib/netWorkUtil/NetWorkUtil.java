...
public class NetWorkUtil {
     ...
     * 网络状态监听器
     */
    static BroadcastReceiver mNetworkStateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (connManager != null){
                ...
                if (State.CONNECTED == wifiState && wifiState != null) { // 判断是否正在使用WIFI网络
                    netState = NET_WIFI;
                    DebugLogs.i("mNetworkStateReceiver 网络是WIFI");
                    return;
                } else if (wifiState != null && mobileState != null
                        && State.CONNECTED != wifiState
                        && State.CONNECTED == mobileState) { //解决没有2G/3G网络时候空指针崩溃问题
                    netState = NET_MOBILE;
                    DebugLogs.i("mNetworkStateReceiver 网络是2G/3G");
                    return;
                } else if (wifiState != null && mobileState != null
                        && State.CONNECTED != wifiState
                        && State.CONNECTED != mobileState) {
                    netState = NET_NONE;
                    DebugLogs.i("mNetworkStateReceiver 当前无网络!");
                    ToastUtils.showToast(context, "Network connection failure.");
                }
            }
        }
    };
    ...
    public static void startNetWorkReceiver(Context context) {
        ...
        DebugLogs.i("registerReceiver mNetworkStateReceiver.");
        context.registerReceiver(mNetworkStateReceiver, filter);
    }

    public static void stopNetWorkReceiver(Context context) {
        DebugLogs.i("unregisterReceiver mNetworkStateReceiver.");
        context.unregisterReceiver(mNetworkStateReceiver); // 取消监听
    }
     ...
     *
     * @param con
     * @return boolean: false时，弹出对话框,点"确定"转向系统设置界面；“取消”直接退出程序
     */
    @SuppressWarnings("static-access")
    public static boolean CheckConnectionOther(final Context con) {
        ConnectivityManager manager = (ConnectivityManager) con.getSystemService(con.CONNECTIVITY_SERVICE);
        NetworkInfo networkinfo = manager.getActiveNetworkInfo();
        if (networkinfo == null || !networkinfo.isAvailable()) {
            // 当前网络不可用
            Dialog dialog = new AlertDialog.Builder(con).setTitle("设置网络")// 设置标题
                    .setMessage("无可用网络连接，请检查手机网络设置或尝试重启手机！")// 设置内容
