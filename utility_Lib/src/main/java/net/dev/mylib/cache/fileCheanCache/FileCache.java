package net.dev.mylib.cache.fileCheanCache;
...
public class FileCache {
    ...
    public static String getMemoryInfo(Context con) {
        String rs = "";
        try {
            ActivityManager activityManager = (ActivityManager) con.getSystemService(con.ACTIVITY_SERVICE);
            ActivityManager.MemoryInfo outInfo = new ActivityManager.MemoryInfo();
            activityManager.getMemoryInfo(outInfo);
            rs = "剩余内存:---->" + (outInfo.availMem >> 20) + " M , " + "---> 是否处于低内存状态: " + outInfo.lowMemory + ", 总内存: " + (outInfo.totalMem >> 20) + " M";
        } catch (Exception e) {
        }
        return rs;
    }
    ...
    /**
     * 获取当前私有目录
     * return /data/data/com.mt.mtpp/files
     */
